...
import { Solution, SocraticResponse } from '../lib/gemini'
import { solveProblem, resolveQuestionText } from '@/lib/solveProblem'
...
export default function ScanScreen() {
  ...
  const [autoSolveAfterCapture, setAutoSolveAfterCapture] = useState(false)
  const [readingPhoto, setReadingPhoto] = useState(false)
  const [explanationStyle, setExplanationStyle] = useState<ExplanationStyleId>(DEFAULT_EXPLANATION_STYLE)
  ...
  // After camera/gallery: read photo and solve automatically — no typing required
  useEffect(() => {
    if (!autoSolveAfterCapture || !image) return
    setAutoSolveAfterCapture(false)
    solve()
  }, [autoSolveAfterCapture, image?.uri])
  ...
  const solve = async () => {
    ...
    try {
      const { result, extractedText: resolved } = await solveProblem({
        questionText: text,
        imageUri: image?.uri,
      ...
      if (resolved) setExtractedText(resolved)
  ...
  if (screen === 'input') {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(150).springify()} style={styles.styleSection}>
          <Text style={styles.styleSectionLabel}>Explain like my teacher</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.styleScroll}>
            {EXPLANATION_STYLES.map((st) => (
              <TouchableOpacity
                key={st.id}
                style={[styles.styleChip, explanationStyle === st.id && styles.styleChipActive]}
                onPress={() => handlePress(() => setExplanationStyle(st.id))}
              >
                <Text style={styles.styleChipEmoji}>{st.emoji}</Text>
                <Text
                  style={[
                    styles.styleChipText,
                    explanationStyle === st.id && styles.styleChipTextActive,
                  ]}
                >
                  {st.label}
                </Text>
        ...
        {inputTab === 'camera' && (
          <Animated.View entering={FadeInUp.duration(600).delay(300).springify()} style={styles.cameraOptions}>
            <LinearGradient
              colors={['rgba(14,165,164,0.25)', 'rgba(167,139,250,0.12)', 'transparent']}
              style={styles.cameraHero}
            >
              <Text style={styles.cameraHeroEmoji}>📐</Text>
              <Text style={styles.cameraHeroTitle}>Snap → we read & solve</Text>
              <Text style={styles.cameraHeroSub}>
                No typing needed. Take a photo of any homework question and we handle the rest.
              </Text>
              <View style={styles.subjectChips}>
                {['Math', 'Physics', 'Chem', 'History'].map((label) => (
                  <View key={label} style={styles.subjectChip}>
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <Animated.View entering={FadeInDown.duration(600).springify()} style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Fix & retry</Text>
        ...
        <Animated.View entering={FadeInUp.duration(600).delay(300).springify()} style={styles.previewBottom}>
          ...
          <Text style={styles.previewHint}>
            We could not read the photo clearly. You can fix the text below or retake the picture.
          </Text>
          <TextInput
            style={styles.fixInput}
            value={typedText || extractedText}
            onChangeText={setTypedText}
            placeholder="Type what the problem says (only if needed)..."
    ...
    return (
      <SafeAreaView style={[styles.container, styles.centerContent]}>
        ...
        <Text style={styles.solvingSubtitle}>
          {readingPhoto ? 'Reading your photo and solving…' : 'AI is analyzing your problem'}
        </Text>
        ...
        <ScrollView style={styles.scrollArea} showsVerticalScrollIndicator={false}>
          ...
          {extractedText ? (
            <Animated.View
              ...
              style={[styles.extractedBox, { marginHorizontal: 16, marginTop: 8 }]}
            >
              <Text style={styles.extractedLabel}>Read from your photo</Text>
          ...
          {solution && (
            <>
              ...
              {/* Fun fact */}
              {solution.real_world_use ? (
                <Animated.View
                  entering={FadeInUp.duration(600).delay(580 + (solution?.steps?.length || 0) * 100).springify()}
                  style={styles.realWorldCard}
                >
                  <Text style={styles.realWorldLabel}>🌍 Where will I use this?</Text>
                  <Text style={styles.realWorldText}>{solution.real_world_use}</Text>
                </Animated.View>
              ) : null}
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handlePress(() => {
                          ...
                          recordLearningEvent({
                            type: 'hint_reveal',
                            topic: socratic.topic,
                            subject: socratic.subject,
                          })
                        })
                      }
                    >
...
const styles = StyleSheet.create({
  ...
  styleSection: { paddingHorizontal: 16, marginBottom: 12, gap: 8 },
  styleSectionLabel: {
    fontSize: 11,
    fontWeight: '700',
    color: '#666',
  ...
  styleChipText: { fontSize: 12, color: '#888', fontWeight: '600' },
  styleChipTextActive: { color: ACCENT },
  realWorldCard: {
    marginHorizontal: 16,
    marginTop: 12,
  ...
  realWorldLabel: { fontSize: 11, color: '#60a5fa', fontWeight: '700', letterSpacing: 0.5 },
  realWorldText: { fontSize: 14, color: '#fff', lineHeight: 20 },
  examplesLabel: {
    color: '#444',
    fontSize: 12,
    fontWeight: '500',
